...
if __name__ == '__main__':
    ...
    for ann in ds.Annotators:
        for doc in ann.documents:
            cursor.execute("Select FirstDataSource from Projects where idProjects=%s",(doc.DatabaseID,))
            results = cursor.fetchone()
            if results!=None and "Digital Social" in results[0]:
                DSI_total = DSI_total + 1
                if doc.isSpam==True:
                    DSI_spam = DSI_spam + 1
            print(doc.DatabaseID)
            print(doc.isSpam)
            number_of_docs = number_of_docs + 1
    print number_of_docs
    print "DSI total:"+str(DSI_total)
    print "DSI spam:" + str(DSI_spam)
