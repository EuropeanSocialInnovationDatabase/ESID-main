...
if __name__ == '__main__':
    ...
    EMBEDDING_DIM = 50
    data_folder = "../../../Helpers/AnnotationData/August"
    ds = DataSet()
    total_num_spam = 0
    sentences = []
    total_num_files = 0
            ...
            for doc1 in annotator1.documents:
                for doc2 in annotator2.documents:
                    if doc1.DocumentName == doc2.DocumentName and doc1.DocumentName not in done_documents:
                        ...
                        if  abs(doc1.Project_Mark_Outputs - doc2.Project_Mark_Outputs)<2:
                            output_agreed = output_agreed + 1
                        else:
                            list_of_unagreed.append(doc1.DocumentName)
                        if abs(doc1.Project_Mark_Objective -  doc2.Project_Mark_Objective)<2:
                            objective_agreed = output_agreed + 1
                        else:
                            list_of_unagreed.append(doc1.DocumentName)
                        if abs(doc1.Project_Mark_Innovativeness - doc2.Project_Mark_Innovativeness) < 2:
                            inovativeness_agreed = inovativeness_agreed + 1
                        else:
                            list_of_unagreed.append(doc1.DocumentName)
                        if abs(doc1.Project_Mark_Actors - doc2.Project_Mark_Actors) < 2:
    ...
    if not os.path.exists("NeedsAnnotation_newM"):
        os.makedirs("NeedsAnnotation_newM")
    for file in ls:
        copyfile(data_folder+"/ZSI1/"+file,"NeedsAnnotation_newM/"+file)
        f = open("NeedsAnnotation_newM/"+file.replace(".txt",".ann"),'w')
        f.close()
