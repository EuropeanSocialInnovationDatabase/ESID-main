...
if __name__ == '__main__':
    ...
    labels = to_categorical(np.asarray(innovativeness))
    ...
    for i in range(1,10):
        ...
        for pred in predictions:
            ...
            if pred[1] > 0.5 and y_val[i][0] == 1:
                TP_sum = TP_sum + 1
            if pred[1] > 0.5 and y_val[i][0]==0:
                FP_sum = FP_sum + 1
            if pred[1] < 0.5 and y_val[i][0]==1:
                FN_sum = FN_sum + 1
            i = i+1
    ...
    with open("../Models/model_innovativeness.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("../Models/model_innovativeness.h5")
