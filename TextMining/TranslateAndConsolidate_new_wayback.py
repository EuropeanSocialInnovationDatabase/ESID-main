...
if __name__ == '__main__':
    ...
    for res in results:
        ...
        documents = mongo_db.crawl20180801_wayback.find({"mysql_databaseID":str(pro.idProject),"page_title":{"$regex":"(A|a)bout|(V|v)ision|(M|m)ision|(C|c)ontact|(K|k)ontakt|(P|p)artner"}},no_cursor_timeout=True).batch_size(100)
        project_text = ""
        original_text = ""
        if documents.count()==0:
            print("Too few documents")
            documents = mongo_db.crawl20180801_wayback.find({"mysql_databaseID": str(pro.idProject)},
        ...
        if project_text == "":
            ...
            continue
        try:
            mongo_db.crawl20180801_wayback_translated.insert_one(
                {
                    "timestamp":time.time(),
                    "relatedTo": "Projects",
