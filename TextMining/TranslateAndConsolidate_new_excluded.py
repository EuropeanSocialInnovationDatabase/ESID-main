...
if __name__ == '__main__':
    ...
    sql_projects = "Select ProjectName,ProjectWebpage,FirstDataSource,DataSources_idDataSources,idProjects from Projects where Exclude=1 and FirstDataSource like '%Digital Social%'"
    ...
    for res in results:
        ...
        if project_text == "":
            ...
            db.commit()
            continue
        if len(project_text)<350:
            continue
        try:
            mongo_db.crawl20180801_excluded_translated.insert_one(
                {
                    "timestamp":time.time(),
                    "relatedTo": "Projects",
            ...
            if not os.path.exists("Excluded_dataset"):
                os.makedirs("Excluded_dataset")
            txt = open("Excluded_dataset/"+str(pro.idProject)+".txt",'w')
            txt.write(project_text)
            txt.close()
            ann = open("Excluded_dataset/"+str(pro.idProject)+".ann",'w')
            ann.write("""Objectives: 0
Actors: 0
Outputs: 0
Innovativenss: 0
SI: 0""")
