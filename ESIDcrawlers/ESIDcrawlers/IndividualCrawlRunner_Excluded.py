...
cursor = db.cursor()
sql = "Select idProjects,ProjectName,ProjectWebpage,FacebookPage from Projects where Exclude=1 and FirstDataSource like '%Digital Social%' order by rand() limit 1000"
cursor.execute(sql)
...
try:
    for res in results:
        if res[2] is not None:
            ...
            command = 'timeout 600 scrapy crawl IndividualSpider_Excluded -a '+str('url='+res[2].encode('utf-8'))+' -a'+' id='+str(res[0])+' -a'+ str(' Name="'+str(res[1].encode('utf-8'))+'"')
            #subprocess.call(command,shell=True)
            os.system(command)
        if res[3] is not None:

            command = 'timeout 600 scrapy crawl IndividualSpider_Excluded -a ' + str(
